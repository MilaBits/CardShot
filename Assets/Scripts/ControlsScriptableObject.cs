...
[CreateAssetMenu(fileName = "New Controls", menuName = "Controls")]
public class ControlsScriptableOBject : ScriptableObject {
    ...
    private void OnValidate() {
        ...
        foreach (var input in Inputs) {
            if (input.CopyInputName) {
                input.Ps4 = input.InputName + $"_{playerNumber}_Ps4";
                input.Xbox = input.InputName + $"_{playerNumber}_Xbox";
                input.Keyboard = input.InputName + $"_{playerNumber}_Keyboard";
            }
    ...
    public class GameInput {
        ...
        [InfoBox("$Ps4Message", InfoMessageType.Info, "ps4Invalid")]
        public string Ps4;

        [HideInInspector]
        public string Ps4Message;

        [HideInInspector]
        public bool ps4Invalid;

        [FoldoutGroup("$InputName/Unity Inputs", false), LabelWidth(65)]
        [InfoBox("$XboxMessage", InfoMessageType.Info, "XboxInvalid")]
        public string Xbox;    

        [HideInInspector]
        public string XboxMessage;

        [HideInInspector]
        public bool XboxInvalid;

        [FoldoutGroup("$InputName/Unity Inputs", false), LabelWidth(65)]
        [InfoBox("$KeyboardMessage", InfoMessageType.Info, "KeyboardInvalid")]
    ...
    [MenuItem("Assets/Game/Controls")]
    public static void CreateAsset() {
        ScriptableObjectUtility.CreateAsset<ControlsScriptableOBject>();
    }
    ...
    public float GetAxisRaw(string input) {
        ...
        foreach (var current in Inputs) {
            if (input.Equals(current.InputName)) {
                ...
                switch (inputMethod) {
                    case InputMethod.Ps4:
                        return Input.GetAxisRaw(gameInput.Ps4);
                    case InputMethod.Keyboard:
                        return Input.GetAxisRaw(gameInput.Keyboard);
                    case InputMethod.Xbox:
                        return Input.GetAxisRaw(gameInput.Xbox);
                    default:
                        throw new ArgumentOutOfRangeException("inputMethod", inputMethod, null);
                }
            }
